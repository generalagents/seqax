...
def mixture_of_gaussians(
    ...
    floats = (
        gen.normal(0, 1, (examples, seq_len)).astype(np.float32) * sample_stddevs
        + sample_centers
    )
...
@jaxtyped(typechecker=typechecker)
def create_text_with_images(
    seq_len: int, examples: int, max_text_token_id: int, gen: np.random.Generator
) -> np.ndarray:
    seq = gen.integers(1, 11, (examples, seq_len), dtype=np.uint32)
    # select random indices to insert images
    image_indices = gen.integers(0, seq_len, (examples, 2), dtype=np.uint32)
    # substitute the image indices with image tokens
    # at each image index choose a token from max_text_token_id to max_text_token_id + 10
    image_tokens = gen.integers(
        max_text_token_id, max_text_token_id + 11, (examples, 2), dtype=np.uint32
    )
    seq[np.arange(examples)[:, None], image_indices] = image_tokens
    return seq
...
def synthetic_task(
    ...
    random_known_distance_copy_data = random_known_distance_copy(
        task_seq_len, examples, gen
    )
    random_unknown_distance_copy_data = random_unknown_distance_copy(
        task_seq_len, examples, gen
    )
    mixture_of_gaussians_data = mixture_of_gaussians(task_seq_len, examples, gen)
    images_data = create_text_with_images(
        task_seq_len, examples, config.max_text_token_id, gen
    )
    tasks = np.asarray(
        [
            copy_data,
            reverse_data,
            random_known_distance_copy_data,
            random_unknown_distance_copy_data,
            mixture_of_gaussians_data,
            images_data,
        ]
    )
    task_id = gen.integers(1, 7, (examples,), dtype=np.uint32)
    targets = np.append(
        task_id[:, np.newaxis], tasks[task_id - 1, np.arange(examples)], axis=1
    )
...
def main(config):
    ...
    for split, mode in [
        (flat_tokens.Split.VALIDATION, "w-"),
        (flat_tokens.Split.TRAIN, "r+"),
    ]:
